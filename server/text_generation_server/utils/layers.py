...
# 用于模型层的owq量化
class OWQLinear(nn.Module):
    def __init__(self, weight, bias, qweight, qzeros, scales, group_size):
        super().__init__()
    ...
    def forward(self, x):
        return owq_cuda.owq_linear(x, self.qweight, self.qzeros, self.scales, self.group_size, self.bias)
        ...
        if HAS_OWQ:
            ...
            # 将owq量化需要用到的参数收集
                ...
                raise NotImplementedError(
                    f"The passed weight is not `owq_gptq` compatible, loader needs to be updated."
                )
            # 需要补充的地方，传入owq量化需要的参数
            linear = None 
        else:
            raise ImportError(
                "Please install OWQ from https://github.com/xvyaward/owq"
            )
    elif quantize == "fp8":
...
class TensorParallelHead(SuperLayer):
    ...
    def load(config, prefix: str, weights):
        ...
        # GPTQ,AWQ,EETQ，OWQ don't quantize heads (nor embeddings)
        if config.quantize in ["gptq", "awq", "eetq","owq"]:
            quantize = None
        else:
